...
from signal_history import DEFAULT_MIN_ENTRY_CONFIDENCE, scanner_result_to_trade_outcome
from trade_repository import utc_iso


EASTERN = ZoneInfo("America/New_York")
FUNNEL_STAGES = (
    "scanned", "valid_results", "directional_candidates", "qualified_setups",
    "armed", "trigger_reached", "trade_entered",
)
...
class AuthoritativeEntryFunnelRepository:
    ...
    def save_cycle(self, *, scanner_id, run_number, started_at, completed_at, symbols, entered_events):
        diagnostics = [classify_symbol(symbol, result) for symbol, result in symbols]
...
def classify_symbol(symbol, result):
    ...
    candidate = scanner_result_to_trade_outcome(result) if valid else None
    qualified = signal in {"BULLISH SETUP", "BEARISH SETUP"}
    ...
    elif score is None or score < DEFAULT_MIN_ENTRY_CONFIDENCE:
    ...
    return {
        ...
        "candidate": candidate is not None, "qualified": qualified, "armed": armed,
        ...
        "score": score, "state": stage, "primary_blocker": blocker,
...
def funnel_counts(rows, *, entered_count=0):
    return {
        ...
        "qualified_setups": sum(row["qualified"] for row in rows),
