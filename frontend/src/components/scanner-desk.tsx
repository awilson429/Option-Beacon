...
export function ScannerDesk() {
  ...
  return <div className="mx-auto max-w-[1680px] px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
    ...
    {response.error?<section className="surface p-5"><SectionError label="Scanner" retry={refresh}/></section>:!response.data?<ScannerLoading/>:<>
      ...
      {response.data.health.data_freshness==="refreshing"&&<div role="status" className="mb-5 flex items-start gap-3 rounded-xl border border-cyan-400/20 bg-cyan-400/[.06] px-4 py-3 text-xs leading-5 text-cyan-100"><Clock3 aria-hidden size={16} className="mt-0.5 shrink-0"/><span><strong>Scanner is refreshing.</strong> The last completed cycle remains historical until the active scan finalizes.</span></div>}
