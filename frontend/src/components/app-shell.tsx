...
const groups = [
  { label:"Trade", items:[["SPY / QQQ",CircleGauge,true],["Scanner",ScanSearch,false],["Active Trades",Activity,false],["Journal",BookOpen,false]] },
  { label:"Analytics", items:[["Performance",ChartNoAxesCombined,false],["Research",SlidersHorizontal,false],["Reports",FileChartColumn,false]] },
  { label:"System", items:[["Alerts",Bell,false],["Data Health",HeartPulse,false],["Settings",Settings,false]] },
] as const;
...
function Navigation({ close }: { close?:()=>void }) { return <nav aria-label="Primary" className="mt-8 space-y-7">{groups.map(group=><div key={group.label}><p className="px-2 text-[9px] font-bold uppercase tracking-[.2em] text-slate-700">{group.label}</p><ul className="mt-2 space-y-1">{group.items.map(([name,Icon,active])=><li key={name}>{active ? <a onClick={close} href="/options" aria-current="page" className="flex items-center gap-3 rounded-lg border border-violet-400/20 bg-violet-400/10 px-3 py-2.5 text-xs font-semibold text-violet-100"><Icon aria-hidden size={15}/><span>{name}</span><ChevronRight aria-hidden className="ml-auto" size={13}/></a> : <span aria-disabled className="flex cursor-not-allowed items-center gap-3 rounded-lg px-3 py-2.5 text-xs font-medium text-slate-600"><Icon aria-hidden size={15}/><span>{name}</span><span className="ml-auto text-[8px] uppercase tracking-widest">Soon</span></span>}</li>)}</ul></div>)}</nav>; }
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="min-h-screen lg:grid lg:grid-cols-[220px_1fr]">
    <aside className="fixed inset-y-0 left-0 z-40 hidden w-[220px] border-r border-slate-800/80 bg-[#080c14] px-4 py-5 lg:block"><Brand/><Navigation/><p className="absolute bottom-5 left-5 text-[9px] uppercase tracking-[.15em] text-slate-700">React migration · Phase 1</p></aside>
