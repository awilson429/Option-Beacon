...
def qqq_command_card_markup(model):
    """Polished compact panes inside the existing client-side pill shell."""
    ...
    def metric(label,value,priority=""):
        return f'<div class="ob-qpol-metric {priority}"><span>{escape(label)}</span><strong>{escape(str(value))}</strong></div>'
    ...
    chips="".join(f'<span class="ob-qpol-chip">{escape(chip)}</span>' for chip in model["chips"] if chip and "TRADE #0" not in chip) or '<span class="ob-qpol-chip is-muted">Context unavailable</span>'
    ...
    session="".join(f'<div class="ob-qpol-tile">{metric(label,value,"is-primary" if label=="P&L" else "")}</div>' for label,value in (("Trades",pulse["closed_trades"]),("W/L",f'{pulse["wins"]} / {pulse["losses"]}'),("Win Rate",_fmt(pulse["win_rate"],"percent")),("P&L",_fmt(pulse["total_pnl"],"money")),("Avg",_fmt(pulse["average_trade"],"money")),("Best",_fmt(pulse["best_trade"],"money")),("Worst",_fmt(pulse["worst_trade"],"money"))))
    ...
    if ft:
        ...
        first_two=f'<span class="ob-qpol-chip">{escape(ft["governance"])}</span><strong>{shadow["accepted_trades"]}/50 accepted</strong><span>{shadow["sessions"]}/20 sessions</span><span>{_fmt(shadow["expectancy"],"money")} expectancy</span>'
    else:first_two='<span class="ob-qpol-chip is-muted">AWAITING SAMPLE</span><span>Awaiting post-deployment sample</span>'
    marks=(f'<strong>{coverage["positions_with_1_or_more_marks"]} marked</strong><span>{_ratio(coverage["average_marks_per_trade"])} avg/trade · {len(coverage["missing_mark_trades"])} missing</span>' if coverage.get("positions_with_1_or_more_marks") else '<span class="ob-qpol-chip is-muted">AWAITING OBSERVATIONS</span>')
    default="overview" if model["market_open"] else "session";checked=lambda name:' checked' if name==default else ''
    return f'''<style>.ob-qpol{{background:#101722;border:1px solid #273344;border-radius:14px;color:#e8edf3;padding:10px 12px}}.ob-qpol-head{{align-items:center;display:flex;gap:10px;justify-content:space-between}}.ob-qpol-id{{align-items:baseline;display:flex;flex-wrap:wrap;gap:5px 11px}}.ob-qpol-symbol{{font-size:1rem;font-weight:850}}.ob-qpol-price{{font-size:1.38rem;font-weight:850}}.ob-qpol-bias{{font-size:.76rem;font-weight:800}}.ob-qpol-context{{color:#a8b3bf;font-size:.7rem}}.ob-qpol-status,.ob-qpol-chip{{background:#1b2735;border-radius:999px;font-size:.6rem;font-weight:750;padding:3px 7px}}.ob-qpol-status{{border:1px solid #3b526b;white-space:nowrap}}.ob-qpol-time,.ob-qpol-muted{{color:#82909f;font-size:.59rem}}.ob-qpol>input{{height:0;opacity:0;position:absolute;width:0}}.ob-qpol-nav{{background:#151f2b;border-radius:999px;display:flex;gap:2px;margin-top:7px;padding:2px;width:max-content;max-width:100%}}.ob-qpol-nav label{{border-radius:999px;color:#8d9aa8;cursor:pointer;font-size:.59rem;font-weight:800;padding:4px 10px}}#qpol-overview:checked~.ob-qpol-nav label[for=qpol-overview],#qpol-session:checked~.ob-qpol-nav label[for=qpol-session],#qpol-edge:checked~.ob-qpol-nav label[for=qpol-edge],#qpol-research:checked~.ob-qpol-nav label[for=qpol-research]{{background:#263548;color:#f0f3f6}}.ob-qpol-pane{{display:none;padding-top:7px}}#qpol-overview:checked~.ob-qpol-panes .is-overview,#qpol-session:checked~.ob-qpol-panes .is-session,#qpol-edge:checked~.ob-qpol-panes .is-edge,#qpol-research:checked~.ob-qpol-panes .is-research{{display:block}}.ob-qpol-group-grid{{display:grid;gap:7px;grid-template-columns:1.5fr 1fr .7fr}}.ob-qpol-group,.ob-qpol-tile{{background:#151f2b;border:1px solid #263443;border-radius:9px;padding:7px 8px}}.ob-qpol-group h4,.ob-qpol-pane>h4{{color:#8e9baa;font-size:.59rem;letter-spacing:.07em;margin:0 0 5px}}.ob-qpol-metrics{{display:grid;gap:6px;grid-template-columns:repeat(5,minmax(0,1fr))}}.ob-qpol-metric span{{color:#82909f;display:block;font-size:.53rem;text-transform:uppercase}}.ob-qpol-metric strong{{font-size:.72rem;overflow-wrap:anywhere}}.ob-qpol-session{{display:grid;gap:6px;grid-template-columns:repeat(7,minmax(0,1fr))}}.ob-qpol-session .ob-qpol-tile{{padding:7px}}.ob-qpol-session .is-primary strong{{font-size:1rem}}.ob-qpol-first{{align-items:center;background:#151f2b;border-radius:8px;display:flex;flex-wrap:wrap;gap:6px 14px;margin-top:6px;padding:6px 8px}}.ob-qpol-first span,.ob-qpol-first strong{{font-size:.65rem}}.ob-qpol-edge{{display:grid;gap:6px;grid-template-columns:repeat(4,minmax(0,1fr))}}.ob-qpol-edge .ob-qpol-tile{{padding:7px 8px}}.ob-qpol-research{{display:grid;gap:7px;grid-template-columns:repeat(3,minmax(0,1fr))}}.ob-qpol-research .ob-qpol-group>div{{display:flex;flex-direction:column;gap:4px;font-size:.65rem}}.ob-qpol-chips{{display:flex;flex-wrap:wrap;gap:4px}}.ob-qpol-chip.is-muted{{color:#82909f}}.ob-qpol-dna{{grid-column:1/-1;padding:2px 2px 0}}.ob-qpol-dna summary{{cursor:pointer;font-size:.63rem;font-weight:800}}@media(max-width:850px){{.ob-qpol-group-grid,.ob-qpol-research{{grid-template-columns:repeat(2,minmax(0,1fr))}}.ob-qpol-session,.ob-qpol-edge{{grid-template-columns:repeat(4,minmax(0,1fr))}}}}@media(max-width:560px){{.ob-qpol-head{{align-items:flex-start;flex-direction:column}}.ob-qpol-nav{{overflow-x:auto;width:100%}}.ob-qpol-nav label{{flex:1;text-align:center;white-space:nowrap}}.ob-qpol-group-grid,.ob-qpol-research{{grid-template-columns:minmax(0,1fr)}}.ob-qpol-session,.ob-qpol-edge,.ob-qpol-metrics{{grid-template-columns:repeat(2,minmax(0,1fr))}}}}</style><section class="ob-qpol"><div class="ob-qpol-head"><div><div class="ob-qpol-id"><span class="ob-qpol-symbol">QQQ</span><strong class="ob-qpol-price">{escape(_fmt(model["price"],"price"))}</strong><span class="ob-qpol-bias">{escape(model["bias"])} BIAS</span><span class="ob-qpol-context">{escape(_fmt(model["regime"]))} · {escape(_fmt(model["setup"]))}</span></div><span class="ob-qpol-time">{escape(format_card_timestamp(model.get("updated_at")))}</span></div><span class="ob-qpol-status">{escape(model["status"])}</span></div><input type="radio" name="qqq-command-view" id="qpol-overview"{checked("overview")}><input type="radio" name="qqq-command-view" id="qpol-session"{checked("session")}><input type="radio" name="qqq-command-view" id="qpol-edge"{checked("edge")}><input type="radio" name="qqq-command-view" id="qpol-research"{checked("research")}><nav class="ob-qpol-nav"><label for="qpol-overview">OVERVIEW</label><label for="qpol-session">SESSION</label><label for="qpol-edge">EDGE</label><label for="qpol-research">RESEARCH</label></nav><div class="ob-qpol-panes"><div class="ob-qpol-pane is-overview"><div class="ob-qpol-group-grid"><div class="ob-qpol-group"><h4>SETUP</h4><div class="ob-qpol-metrics">{setup}</div></div><div class="ob-qpol-group"><h4>CONTEXT</h4><strong>{escape(quality["label"])}{f' · {quality["score"]}/100' if quality["score"] is not None else ''}</strong><div class="ob-qpol-chips">{chips}</div></div><div class="ob-qpol-group"><h4>SESSION POSITION</h4><strong>{escape(sequence)}</strong><div class="ob-qpol-first">{first_two}</div></div></div></div><div class="ob-qpol-pane is-session"><h4>{'SESSION ACTIVE' if model['market_open'] else 'SESSION COMPLETE'}</h4><div class="ob-qpol-session">{"".join(f'<div class="ob-qpol-tile">{item}</div>' for item in [session[i:j] for i,j in []]) if False else session}</div><div class="ob-qpol-first"><strong>FIRST TWO FORWARD TEST</strong>{first_two}</div></div><div class="ob-qpol-pane is-edge"><div class="ob-qpol-edge">{"".join(f'<div class="ob-qpol-tile">{metric(*item)}</div>' for item in (("Closed Trades",edge["closed_trades"]),("Expectancy",_fmt(edge["expectancy"],"money")),("Profit Factor",_ratio(edge["profit_factor"])),("Win Rate",_fmt(edge["win_rate"],"percent")),("Avg Winner",_fmt(edge["average_winner"],"money")),("Avg Loser",_fmt(edge["average_loser"],"money")),("Payoff",_ratio(edge["payoff_ratio"],"x")),("Profitable Sessions",_fmt(edge["profitable_session_percentage"],"percent"))))}</div><span class="ob-qpol-muted">Historical / forward research metrics only</span></div><div class="ob-qpol-pane is-research"><div class="ob-qpol-research"><div class="ob-qpol-group"><h4>CONTEXT</h4><div><span class="ob-qpol-chip">{escape(quality["label"])}</span><span>Evidence availability remains descriptive.</span></div></div><div class="ob-qpol-group"><h4>FIRST_TWO</h4><div>{first_two}</div></div><div class="ob-qpol-group"><h4>MARK TELEMETRY</h4><div>{marks}</div></div><details class="ob-qpol-dna"><summary>QQQ DNA</summary><p class="ob-qpol-muted">CALL/PUT, 0DTE/1DTE, time, sequence, re-entry, spread, setup, regime, MFE/MAE and FIRST_TWO attribution remain descriptive research only.</p></details></div></div></div></section>'''


def _pill_v1_qqq_command_card_markup(model):
