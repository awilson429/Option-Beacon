...
def qqq_command_card_markup(model, *, view="LIVE / SESSION"):
    ...
    setup=f'<strong class="ob-qnative-contract">{escape(contract)}</strong><div class="ob-qnative-setup-lead"><strong>{escape(model["bias"])} BIAS</strong><span>Trigger {escape(_fmt(model["trigger"],"price"))}</span></div><div class="ob-qnative-setup-metrics">'+"".join(metric(*item) for item in setup_items)+"</div>"
